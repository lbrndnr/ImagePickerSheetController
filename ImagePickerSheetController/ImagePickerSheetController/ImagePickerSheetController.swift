...
public class ImagePickerSheetController: UIViewController, UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIViewControllerTransitioningDelegate {
    ...
    private var didCheckForAuthorization = false
    ...
    public override func viewWillAppear(animated: Bool) {
        ...
        if isAuthorized {
            ...
            view.hidden = true
            if !didCheckForAuthorization {
                didCheckForAuthorization = true
                checkForAuthorization()
            }
        }
    }

    private func checkForAuthorization(animate: Bool = false) {
        ...
        case .Authorized:
            ...
            if animate {
                ...
                tableView.transform = CGAffineTransformMakeTranslation(0, self.tableView.bounds.size.height)
                tableView.reloadData()
                UIView.animateWithDuration(animate ? AnimationController.animationDuration : 0,
                    animations: {
        ...
        case .NotDetermined:
            PHPhotoLibrary.requestAuthorization({
                ...
                dispatch_async(dispatch_get_main_queue(), {
                    self.checkForAuthorization(animate: status == PHAuthorizationStatus.Authorized)
                })
            })

        case .Denied:
            let alertController = UIAlertController(title: NSLocalizedString("Photos Access Denied", comment: "Photos Access Denied alert title"),
                message: NSLocalizedString("Access to photos has been denied. Please allow access by opening the Settings App and going to Privacy.", comment: "Photos Access Denied alert message"),
                preferredStyle: UIAlertControllerStyle.Alert)
            let cancelAction = UIAlertAction(title: NSLocalizedString("OK", comment: "OK alert button label"),
        ...
        case .Restricted:
            let alertController = UIAlertController(title: NSLocalizedString("Photos Access Restricted", comment: "Photos Access Restricted alert title"),
                message: NSLocalizedString("Access to photos has been restricted by either a parental control or configuration policy in place.", comment: "Photos Access Restricted alert message"),
                preferredStyle: UIAlertControllerStyle.Alert)
            let cancelAction = UIAlertAction(title: NSLocalizedString("OK", comment: "OK alert button label"),
                style: UIAlertActionStyle.Cancel,
                handler: {
